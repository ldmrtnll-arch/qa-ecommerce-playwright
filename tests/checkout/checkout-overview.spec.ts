...
test.describe('Checkout overview', () => {
  ...
  test.beforeEach(async ({ authenticatedPage }) => {
    ...
    const checkoutInformationPage =
      new CheckoutInformationPage(authenticatedPage);
    ...
    await expect(cartProductPrice).toHaveText(
      /^\$\d+\.\d{2}$/,
    );
    ...
    expectedProductData = {
      description: (
        await cartProductDescription.innerText()
      ).trim(),

      price: (
        await cartProductPrice.innerText()
      ).trim(),

      quantity: (
        await cartProductQuantity.innerText()
      ).trim(),
    };
  ...
  test('TC-CHK2-001 - should display the selected product in the checkout overview', async () => {
    ...
    await expect(
      checkoutOverviewPage.getProductDescription(
        overviewProduct,
      ),
    ...
    await expect(
      checkoutOverviewPage.getProductQuantity(
        overviewProduct,
      ),
  ...
  test('TC-CHK2-002 - should calculate the item subtotal correctly', async () => {
    const productPrice = Number(
      expectedProductData.price.replace('$', ''),
    );

    const productQuantity = Number(
      expectedProductData.quantity,
    );
  ...
  test('TC-CHK2-003 - should calculate the order total correctly', async () => {
    await expect(
      checkoutOverviewPage.itemTotalLabel,
    ).toHaveText(/^Item total: \$\d+\.\d{2}$/);

    await expect(
      checkoutOverviewPage.taxLabel,
    ).toHaveText(/^Tax: \$\d+\.\d{2}$/);

    await expect(
      checkoutOverviewPage.totalLabel,
    ).toHaveText(/^Total: \$\d+\.\d{2}$/);

    const subtotalText =
      await checkoutOverviewPage.itemTotalLabel.innerText();

    const taxText =
      await checkoutOverviewPage.taxLabel.innerText();

    const totalText =
      await checkoutOverviewPage.totalLabel.innerText();
  ...
  test('TC-CHK2-005 - should finish a valid purchase successfully', async ({
    ...
    await expect(
      checkoutCompletePage.confirmationHeader,
